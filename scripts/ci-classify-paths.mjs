...
export function classifyPaths(paths) {
  ...
  const docsOnly = files.length > 0 && files.every((p) => p.startsWith('docs/') || p.endsWith('.md'));
  ...
  const unknownCode = !docsOnly && files.length > 0 && !migrations && !boardE2e && !homepageE2e;
...
if (process.argv[1] && import.meta.url === new URL(`file://${process.argv[1]}`).href) {
  ...
  if (!inputPath) throw new Error('usage: node scripts/ci-classify-paths.mjs <changed-files.txt>');
