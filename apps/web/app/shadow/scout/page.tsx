...
export default function ScoutReportPage() {
  ...
  return (
    <main className="min-h-screen bg-[var(--hide-950)] text-[var(--bone-200)]">
      {/* HEADER */}
      <header className="border-b-4 border-[var(--patina-700)] bg-[var(--hide-900)] px-4 py-4 md:px-8">
        <div className="mx-auto flex max-w-5xl items-center justify-between gap-4">
          <div>
            <p className="text-[10px] font-mono uppercase tracking-[0.3em] text-[var(--locked)]">SHADOW Intelligence</p>
            <h1 className="font-display text-2xl font-black tracking-tight text-[var(--bone-200)]">Scout Reports</h1>
            <p className="mt-1 text-xs text-[var(--bone-400)]">Profile intelligence, Recovery Round jobs, and The Scorecard</p>
          </div>
          <div className="flex gap-3">
            <Link
              href="/shadow"
              className="border-2 border-[var(--hide-500)] bg-[var(--hide-900)] px-3 py-2 text-xs font-mono text-[var(--bone-400)] transition hover:border-[var(--patina-700)] hover:text-[var(--bone-200)]"
            >
              ← SHADOW
            </Link>
            <button
              ...
              className="border-2 border-[var(--hide-500)] bg-[var(--hide-900)] px-4 py-2 text-xs font-mono font-bold text-[var(--bone-400)] opacity-60"
            >
      ...
      <div className="mx-auto max-w-5xl p-4 md:p-8 space-y-6">
        {error ? (
          <p className="border border-[var(--locked)] bg-[var(--rust-900)] px-4 py-2 text-xs font-mono text-[var(--locked)]">{error}</p>
        ) : null}

        {/* METRICS DASHBOARD */}
        {scoreboard ? (
          <section className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-5 space-y-4">
            <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-[var(--brass-300)]">Metrics Dashboard — {scoreboard.period}</p>

            {/* Tier Distribution */}
            <div className="grid md:grid-cols-2 gap-4">
              <div>
                <p className="text-xs font-mono text-[var(--bone-400)] mb-2">Tier Distribution</p>
                <div className="space-y-2">
                  {[
                    ...
                    <div key={label} className="space-y-1">
                      <div className="flex justify-between text-[9px] text-[var(--bone-400)]">
                        <span>{label} {count} users</span>
                        <span>{percent}%</span>
                      </div>
                      <div className="h-1.5 bg-[var(--hide-950)] border border-[var(--rust-700)] overflow-hidden">
                        <div
              ...
              {/* Effectiveness */}
              <div>
                <p className="text-xs font-mono text-[var(--bone-400)] mb-2">Effectiveness Metrics</p>
                <div className="space-y-2">
                  {[
                    ...
                    <div key={label} className="flex justify-between border border-[var(--rust-700)] bg-[var(--hide-950)] px-3 py-2">
                      <span className="text-[9px] text-[var(--bone-400)]">{label}</span>
            ...
            {/* Engagement Summary */}
            <div className="border-t border-[var(--rust-700)] pt-3">
              <p className="text-xs font-mono text-[var(--bone-400)] mb-2">Engagement Summary</p>
              <div className="grid md:grid-cols-3 gap-2">
                {[
                  ...
                  <div key={label} className="border border-[var(--rust-700)] bg-[var(--hide-950)] px-3 py-2 text-center">
                    <p className="text-[9px] text-[var(--bone-400)]">{label}</p>
                    <p className="mt-1 text-sm font-mono font-bold text-[var(--brass-300)]">{value}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            {scoreboard.effectiveness.concernedTopics.length > 0 ? (
              <div className="border-t border-[var(--rust-700)] pt-3">
                <p className="text-xs font-mono text-[var(--bone-400)] mb-2">Top Engaged Topics</p>
                <div className="flex flex-wrap gap-2">
                  {scoreboard.effectiveness.concernedTopics.map((topic, idx) => (
                    <span key={topic} className="border border-[var(--hide-500)] bg-[var(--hide-950)] px-3 py-1 text-[9px] font-mono text-[var(--brass-300)]">
                      #{idx + 1} {topic}
                    </span>
                  ))}
                </div>
        ...
        {/* THE SCORECARD */}
        {scoreboard ? (
          <section className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-5">
            <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-[var(--brass-300)]">The Scorecard — {scoreboard.period}</p>
            <div className="mt-4 grid grid-cols-2 gap-3 md:grid-cols-4">
              {[
                ...
                <div key={label} className="border border-[var(--hide-500)] bg-[var(--hide-950)] p-3 text-center">
                  <p className="text-xs font-mono text-[var(--bone-400)]">{label}</p>
                  <p className="mt-1 text-xl font-black text-[var(--brass-300)]">{value}</p>
                </div>
              ))}
            </div>
            <div className="mt-4 flex gap-4">
              {[
                ...
                <div key={label} className="text-xs font-mono text-[var(--bone-400)]">
                  <span className="text-[var(--brass-300)]">{label}</span> {count} users
                </div>
              ))}
              {scoreboard.effectiveness.concernedTopics.length > 0 ? (
                <div className="text-xs font-mono text-[var(--bone-400)]">
                  Topics needing review: <span className="text-[var(--brass-300)]">{scoreboard.effectiveness.concernedTopics.join(', ')}</span>
                </div>
              ) : null}
            </div>
          </section>
        ...
        <section className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-5">
          <div className="flex items-center justify-between gap-3">
            <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-[var(--brass-300)]">
              Scout Reports ({scoutJobs.length})
            </p>
          </div>

          <div className="mt-3 border border-[var(--hide-500)] bg-[var(--hide-950)] p-3">
            ...
            <div className="mt-2 flex flex-wrap gap-2">
              <input
                ...
                className="min-w-[220px] flex-1 border border-[var(--hide-500)] bg-[var(--hide-900)] px-3 py-2 text-xs text-[var(--bone-300)] outline-none focus:border-[var(--brass-500)]"
              ...
              {canViewOrgMetrics ? (
                <button
                  ...
                  disabled={generateBusy}
                  className="border-2 border-[var(--hide-500)] bg-[var(--hide-900)] px-3 py-2 text-[10px] font-mono font-bold uppercase tracking-[0.12em] text-[var(--brass-300)] transition hover:border-[var(--brass-500)] disabled:opacity-50"
                >
                  Generate board summary
                </button>
              ) : null}
            </div>
            {generateNotice ? <p role="status" className="mt-2 text-xs text-[var(--brass-300)]">{generateNotice}</p> : null}
            {generateError ? <p role="alert" className="mt-2 text-xs text-[var(--locked)]">{generateError}</p> : null}
          </div>
              ...
              {scoutJobs.map((job) => (
                <button
                  ...
                  className={`border bg-[var(--hide-950)] p-4 cursor-pointer transition w-full text-left ${
                    selectedJob?.jobId === job.jobId ? 'border-[var(--brass-500)]' : 'border-[var(--hide-500)] hover:border-[var(--patina-700)]'
                  }`}
                ...
                >
                  <div className="flex items-center justify-between gap-3">
                    <div>
                      ...
                      <p className="mt-1 text-[10px] text-[var(--bone-400)] font-mono">{job.jobId.slice(0, 8)}... · {new Date(job.createdAt).toLocaleDateString()}</p>
                    </div>
                    {safeCompleted(job) && job.output?.profileTier ? (
                      <span className="text-[10px] font-mono text-[var(--brass-300)] border border-[var(--hide-500)] px-2 py-0.5">
                  ...
                  {selectedJob?.jobId === job.jobId && safeCompleted(job) && job.output ? (
                    <div className="mt-4 space-y-3 text-xs">
                      {job.output.summary ? (
                        <div>
                          <p className="font-mono text-[var(--brass-300)] uppercase tracking-[0.1em]">Summary</p>
                          <p className="mt-1 leading-6 text-[var(--bone-300)]">{job.output.summary}</p>
                        </div>
                      ) : null}
                      {job.output.strengths?.length ? (
                        <div>
                          <p className="font-mono text-[var(--brass-300)] uppercase tracking-[0.1em]">Strengths</p>
                          <ul className="mt-1 space-y-1 text-[var(--bone-300)]">
                      ...
                      {job.output.growthAreas?.length ? (
                        <div>
                          <p className="font-mono text-[var(--brass-300)] uppercase tracking-[0.1em]">Growth Areas</p>
                          <ul className="mt-1 space-y-1 text-[var(--bone-300)]">
                      ...
                      {job.output.recommendedTopics?.length ? (
                        <div>
                          <p className="font-mono text-[var(--brass-300)] uppercase tracking-[0.1em]">Recommended Topics</p>
                          <div className="mt-1 flex flex-wrap gap-2">
                            {job.output.recommendedTopics.map((t) => (
                              <span key={t} className="border border-[var(--hide-500)] px-2 py-0.5 text-[9px] text-[var(--bone-400)]">{t}</span>
                            ))}
                          </div>
                        </div>
                      ) : null}
                      {job.output.insightNotes ? (
                        <div>
                          <p className="font-mono text-[var(--brass-300)] uppercase tracking-[0.1em]">Insight Notes</p>
                          <p className="mt-1 leading-6 text-[var(--bone-300)]">{job.output.insightNotes}</p>
                        </div>
                      ) : null}
                    </div>
                  ) : null}
                  ...
                  {selectedJob?.jobId === job.jobId
                    && job.status === 'completed'
                    && !safeCompleted(job) ? (
                      <p className="mt-2 text-[10px] font-mono text-[var(--brass-300)]">
                  ...
                  {selectedJob?.jobId === job.jobId && job.status === 'pending' ? (
                    <p className="mt-2 text-[10px] font-mono text-[var(--bone-400)]">Queued for secure background processing.</p>
                  ) : null}

                  {selectedJob?.jobId === job.jobId && job.status === 'failed' ? (
                    <p className="mt-2 text-[10px] font-mono text-[var(--locked)]">Error: {job.error}</p>
                  ) : null}
        ...
        {canViewOrgMetrics && boardJobs.length > 0 ? (
          <section className="border-2 border-[var(--hide-500)] bg-[var(--hide-900)] p-5">
            <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-[var(--brass-300)]">
              Board Summaries ({boardJobs.length})
            </p>
            <div className="mt-4 space-y-3">
              {boardJobs.slice(0, 10).map((job) => (
                <div key={job.jobId} className="border border-[var(--hide-500)] bg-[var(--hide-950)] p-4">
                  <div className="flex items-center justify-between gap-3">
                    <StatusBadge status={job.status} />
                    <p className="text-[10px] text-[var(--bone-400)] font-mono">{job.jobId.slice(0, 8)}… · {new Date(job.createdAt).toLocaleDateString()}</p>
                  </div>
                  {safeCompleted(job) && typeof job.output?.summary === 'string' ? (
                    <p className="mt-3 whitespace-pre-wrap text-xs leading-6 text-[var(--bone-300)]">{job.output.summary}</p>
                  ) : job.status === 'completed' ? (
                    <p className="mt-3 text-xs text-[var(--bone-400)] font-mono">Result withheld or unavailable — no validated summary to display.</p>
                  ) : null}
                </div>
              ))}
            </div>
        ...
        {heavyBagJobs.length > 0 ? (
          <section className="border-2 border-[var(--hide-500)] bg-[var(--hide-900)] p-5">
            <p className="text-[10px] font-mono uppercase tracking-[0.2em] text-[var(--bone-400)]">
            ...
            <div className="mt-3 space-y-2">
              {heavyBagJobs.slice(0, 10).map((job) => (
                <div
                  key={job.jobId}
                  className="flex items-center justify-between border border-[var(--rust-700)] bg-[var(--hide-950)] px-3 py-2"
                >
                  <div className="flex items-center gap-3">
                    <StatusBadge status={job.status} />
                    <span className="text-[10px] font-mono text-[var(--bone-400)]">{new Date(job.createdAt).toLocaleString()}</span>
                  </div>
...
function StatusBadge({ status }: { readonly status: JobStatusResult['status'] }) {
  const styles: Record<JobStatusResult['status'], string> = {
    pending:   'text-[var(--bone-400)] border-[var(--hide-500)]',
    running:   'text-[var(--brass-300)] border-[var(--brass-500)]',
    completed: 'text-[var(--cleared)] border-[var(--cleared)]',
    failed:    'text-[var(--locked)] border-[var(--locked)]',
    cancelled: 'text-[var(--bone-400)] border-[var(--hide-500)]',
  };
  return (
