...
/**
 * pilot.audit_events.created_at is an instant, but that does NOT make the
 * viewer's zone the right one to render it in. "Who last corrected this record,
 * and when" describes something that happened at a gym in Punxsutawney, so a
 * coach reading it on a phone set to another zone must not see a shifted time.
 * formatGymStamp pins both the zone and the locale -- see gymTimeDrift.test.ts,
 * which enforces this and which an earlier version of this helper tripped.
 *
 * Falls back to the raw value rather than rendering nothing: an unparseable
 * timestamp on an audit row is worth showing verbatim so it can be chased.
 */
function formatAuditTimestamp(raw: string): string {
  return formatGymStamp(raw) ?? raw;
}
