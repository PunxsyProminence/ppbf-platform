  ...
  return (
    ...
    >
      <div className="space-y-4">
        {errorMessage ? (
          <section className="tactical-alert-critical text-sm">{errorMessage}</section>
        ) : null}

        {!errorMessage && nodes.length === 0 ? (
          <section className="tactical-panel p-4">
            <p className="text-[12px] font-mono uppercase tracking-[0.16em] text-[var(--red-primary)]">Empty State</p>
            <p className="mt-2 text-[14px] text-[var(--gray-dark)]">No SHADOW knowledge projection data exists for this organization yet.</p>
          </section>
        ) : null}
        ...
        {[
          ...
          <section key={group.title} className="tactical-panel p-4">
            <p className="text-[12px] font-mono uppercase tracking-[0.16em] text-[var(--red-primary)]">{group.title}</p>
            {group.items.length === 0 ? (
              <p className="mt-2 text-sm text-[var(--gray-dark)]">No items.</p>
            ) : (
              <div className="mt-3 space-y-2">
                {group.items.map((item) => (
                  <article key={`${item.entity_type}-${item.entity_id}-${item.created_at}`} className="border border-[var(--black)] bg-[var(--canvas-tan-light)] px-3 py-2 text-[14px] text-[var(--black)]">
                    <p className="font-semibold">{item.title}</p>
                    <p className="mt-1 text-xs text-[var(--gray-dark)]">{item.entity_type} / {item.entity_id}</p>
                    <p className="text-xs text-[var(--gray-dark)]">Review State: {item.review_state}</p>
                  </article>
                ))}
              </div>
            )}
