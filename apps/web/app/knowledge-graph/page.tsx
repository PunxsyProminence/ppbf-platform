    ...
    >
      <div className="space-y-4">
        ...
        {!errorMessage && nodes.length === 0 ? (
          <section className="border-2 border-[color:var(--brass-700)] bg-[var(--hide-900)]/60 p-4">
            <p className="text-[12px] font-mono uppercase tracking-[0.16em] text-[color:var(--brass-300)]">Empty State</p>
            <p className="mt-2 text-[14px] text-[color:var(--bone-300)]">No SHADOW knowledge projection data exists for this organization yet.</p>
          </section>
        ) : null}
        ...
        {[
          ...
          <section key={group.title} className="border-2 border-[color:var(--brass-700)] bg-[var(--hide-900)]/60 p-4">
            <p className="text-[12px] font-mono uppercase tracking-[0.16em] text-[color:var(--brass-300)]">{group.title}</p>
            {group.items.length === 0 ? (
              <p className="mt-2 text-sm text-[color:var(--bone-400)]">No items.</p>
            ) : (
              <div className="mt-3 space-y-2">
                {group.items.map((item) => (
                  <article key={`${item.entity_type}-${item.entity_id}-${item.created_at}`} className="border border-[color:var(--hide-600)] bg-[var(--hide-950)] px-3 py-2 text-[14px] text-[color:var(--bone-200)]">
                    <p className="font-semibold">{item.title}</p>
                    <p className="mt-1 text-xs text-[color:var(--bone-300)]">{item.entity_type} / {item.entity_id}</p>
                    <p className="text-xs text-[color:var(--bone-300)]">Review State: {item.review_state}</p>
                  </article>
                ))}
              </div>
            )}
