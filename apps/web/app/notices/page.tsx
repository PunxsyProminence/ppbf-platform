...
const LIFECYCLE_TONE: Record<AnnouncementLifecycle, string> = {
  live: 'border-[var(--status-ready)] bg-[color-mix(in_srgb,var(--cleared)_14%,var(--paper))]',
  scheduled: 'border-[var(--status-warning)] bg-[color-mix(in_srgb,var(--restricted)_14%,var(--paper))]',
  expired: 'border-[var(--gray-medium)] bg-[var(--canvas-tan)]',
  retired: 'border-[var(--accent)] bg-[color-mix(in_srgb,var(--accent)_14%,var(--paper))]',
};
    ...
    <main className="min-h-screen bg-[var(--canvas-tan)] text-[var(--black)]">
      <div className="mx-auto w-full max-w-6xl px-6 py-10 lg:px-10">
        <header className="space-y-3 border-b-[3px] border-[var(--black)] pb-6">
          <p className="text-xs font-mono uppercase tracking-[0.18em] text-[var(--accent-quiet)]">Gym Communications</p>
          ...
          <div className="grid gap-3 md:grid-cols-3">
            {liveByPlacement.map((group) => (
              <article key={group.placement} className="border-2 border-[var(--black)] bg-[var(--canvas-tan)] p-3">
                <p className="text-xs font-mono uppercase tracking-[0.12em] text-[var(--accent-quiet)]">
