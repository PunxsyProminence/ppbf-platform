    ...
    <RoleStandaloneView roleLabel="Athlete Workspace" routeLabel="/athlete/video-analysis" allowedRoles={['athlete']} showShellHeader={false}>
      <div className="space-y-6">
        <header className="border-2 border-[var(--patina-700)] bg-[var(--hide-950)] p-5">
          <p className="text-xs font-mono uppercase tracking-[0.2em] text-[var(--brass-300)]">Film Review</p>
          <h1 className="mt-2 text-3xl font-black text-[var(--bone-100)]">Athlete Film Lane</h1>
          <p className="mt-2 text-sm text-[var(--bone-300)]">Watch footage your coach has released for your sessions.</p>
        </header>
        ...
        <section className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-4">
          <h2 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">Your Film Library</h2>
          {videoError ? <p className="mt-2 text-xs text-[var(--locked-ink)]">{videoError}</p> : null}
          {!videoError && videos.length === 0 ? (
            <p className="mt-3 text-xs text-[var(--bone-300)]">No film yet. Footage appears here once your coach releases it.</p>
          ) : (
              ...
              {videos.map((v) => (
                <div key={v.video_session_id} className="flex items-center justify-between border border-[var(--hide-500)] bg-[var(--hide-950)] p-3">
                  <div>
                    ...
                    <p className="mt-0.5 text-xs text-[var(--bone-300)]">{v.file_name} · {formatBytes(v.file_size_bytes)} · {v.status}</p>
                    <p className="mt-0.5 text-xs text-[var(--bone-400)]">{new Date(v.created_at).toLocaleString()}</p>
                  </div>
                  <button onClick={() => { void openVideo(v.video_session_id); }} disabled={loadingVideoId === v.video_session_id} className="ml-4 border border-[var(--patina-700)] bg-[var(--rust-900)] px-3 py-1 text-xs font-mono text-[var(--brass-300)] disabled:opacity-50">
        ...
        <section className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-4">
          <h2 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">SHADOW Signal Stream</h2>
          <div className="mt-3 space-y-2">
            {observationError ? <p className="text-xs text-[var(--locked-ink)]">{observationError}</p> : null}
            {observations.slice(0, 5).map((item) => (
              <article key={item.id} className="border border-[var(--hide-500)] bg-[var(--hide-950)] p-3">
                <p className="text-sm font-semibold text-[var(--bone-200)]">{item.label}</p>
                <p className="mt-1 text-xs text-[var(--bone-300)]">Review State: {item.review_state}</p>
              </article>
            ))}
        ...
        <section className="border-2 border-[var(--hide-600)] bg-[var(--hide-900)] p-4">
          ...
          <div className="mt-2 grid gap-2 sm:grid-cols-2">
            {['Skill Recognition', 'Technique Scoring'].map((p) => (
              <div key={p} className="border border-[var(--hide-700)] bg-[var(--hide-950)] p-3">
                <p className="text-xs font-semibold text-[var(--bone-400)]">{p}</p>
                <p className="mt-1 text-xs font-mono uppercase tracking-[0.07em] text-[var(--hide-500)]">{ML_PLACEHOLDER}</p>
              </div>
            ))}
          </div>
        </section>
        ...
        <div className="flex flex-wrap gap-3">
          <Link href="/athlete/dashboard" className="border-2 border-[var(--patina-700)] bg-[var(--rust-900)] px-4 py-2 text-xs font-mono text-[var(--brass-300)]">
