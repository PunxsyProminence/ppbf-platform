...
test('a coach cannot withdraw an entry; an unknown status value is a 400', async () => {
  mockRequirePrincipal.mockResolvedValue(principal({ role: 'coach' }));
  expect((await PATCH(patchRequest({ entry_id: 'e-1', status: 'withdrawn' }))).status).toBeGreaterThanOrEqual(400);
  expect(mockWithdraw).not.toHaveBeenCalled();

  mockRequirePrincipal.mockResolvedValue(principal({}));
  expect((await PATCH(patchRequest({ entry_id: 'e-1', status: 'entered' }))).status).toBe(400);
  expect(mockWithdraw).not.toHaveBeenCalled();
});

test('withdrawing a foreign, missing, or already-withdrawn entry is a hidden not-found', async () => {
  mockRequirePrincipal.mockResolvedValue(principal({}));
...
test('a valid withdrawal audits the action and never touches result or lesson_note', async () => {
  mockRequirePrincipal.mockResolvedValue(principal({}));
