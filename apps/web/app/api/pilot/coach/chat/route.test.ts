...
describe('POST /api/pilot/coach/chat', () => {
  test('ignores a client-supplied organizationId and always uses the principal\'s organization', async () => {
    ...
    const res = await POST(
      postRequest({ message: 'hello', athleteId: 'ath-1', organizationId: 'org-attacker' }),
    );

    expect(res.status).toBe(200);
    expect(mockBuildPrompt).toHaveBeenCalledWith('acct-1', 'org-real', 'coach', 'hello');
