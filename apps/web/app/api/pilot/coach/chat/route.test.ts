...
function principal(role: PilotPrincipal['role'] = 'coach'): PilotPrincipal {
  return {
    accountId: 'acct-1',
    role,
...
function postRequest(body: Record<string, unknown>) {
  return new NextRequest('http://localhost/api/pilot/coach/chat', {
    method: 'POST',
...
describe('POST /api/pilot/coach/chat adapter', () => {
  test('drops client organization scope and preserves the requested athlete subject', async () => {
  ...
  test('rejects a role that cannot use the coach adapter', async () => {
