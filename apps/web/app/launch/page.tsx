      ...
      <div className="mx-auto flex min-h-screen w-full max-w-6xl flex-col px-6 py-10 lg:px-10">
        <header className="flex flex-col gap-4 border-b-4 border-[#d4a574] pb-6 md:flex-row md:items-end md:justify-between">
          <div className="space-y-2">
            <p className="text-xs font-mono uppercase tracking-[0.35em] text-[#d4a574]">Jump In Portal</p>
            <h1 className="text-4xl font-black tracking-tight md:text-5xl">Ready to Go</h1>
            <p className="max-w-2xl text-sm leading-6 text-[#b0a095] md:text-base">
              This is where it starts. Main governance checks, all the working surfaces, and everything you need to step in the ring.
            </p>
          </div>
          <div className="border-2 border-[#d4a574] bg-[#1a1a1a] px-4 py-3 text-xs font-mono text-[#d4a574]">
            Status: LIVE
          </div>
        </header>

        <section className="mt-8 grid gap-6 lg:grid-cols-[1.35fr_0.9fr]">
          <div className="border-4 border-[#8b4444] bg-[#0f0f0f] p-6 shadow-2xl shadow-black/60">
            <div className="flex items-center justify-between gap-4 border-b-4 border-[#8b4444] pb-4">
              <h2 className="text-lg font-semibold text-[#e8d7c6]">The Checks</h2>
            ...
            <ul className="mt-5 space-y-3">
              {launchMilestones.map((item) => (
                <li key={item} className="flex gap-3 border-2 border-[#8b4444] bg-[#1a1a1a] p-4 text-sm text-[#e8d7c6]">
          ...
          <div className="grid gap-6">
            <section className="border-4 border-[#8b4444] bg-[#0f0f0f] p-6">
              <h2 className="text-lg font-semibold text-[#e8d7c6]">Quick Access</h2>
              <div className="mt-4 grid gap-3">
                {launchLinks.map((item) => (
                  <Link
                    key={item.href}
                    href={item.href}
                    className="border-2 border-[#8b4444] bg-[#4a4a4a] px-4 py-3 text-sm text-[#e8d7c6] transition hover:border-[#dc2626] hover:bg-[#5a5a5a] hover:text-[#f0e8d8]"
                  >
                    {item.label}
                  </Link>
                ))}
