  ...
  return (
    ...
    >
      <div className="space-y-4">
        {errorMessage ? (
          <section className="border-2 border-[var(--red-primary)] bg-[var(--canvas-tan-light)] p-4 text-sm text-[var(--red-primary)]">{errorMessage}</section>
        ) : null}

        {!errorMessage && items.length === 0 ? (
          <section className="border-2 border-[var(--black)] bg-[var(--canvas-tan-light)] p-4">
            <p className="text-[12px] font-mono uppercase tracking-[0.18em] text-[var(--gray-dark)]">Empty State</p>
            <p className="mt-2 text-[14px] text-[var(--gray-dark)]">No SHADOW research projection items exist for this organization yet.</p>
          </section>
        ) : null}

        <section className="border-2 border-[var(--black)] bg-[var(--canvas-tan-light)] p-4">
          <p className="text-[12px] font-mono uppercase tracking-[0.18em] text-[var(--gray-dark)]">Review State Summary</p>
          <div className="mt-3 grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
            {[
              ...
              <article key={entry.label} className="border border-[var(--black)] bg-[var(--canvas-tan)] p-3">
                <p className="text-[11px] font-mono uppercase tracking-[0.14em] text-[var(--gray-dark)]">{entry.label}</p>
                <p className="mt-2 text-2xl font-black text-[var(--black)]">{entry.value}</p>
              </article>
            ))}
          </div>
        </section>
        ...
        <section className="space-y-3 border-2 border-[var(--black)] bg-[var(--canvas-tan-light)] p-4">
          <p className="text-[12px] font-mono uppercase tracking-[0.18em] text-[var(--gray-dark)]">Research Intake Cards</p>
          {items.map((item) => (
            <article key={item.event_id} className="border border-[var(--black)] bg-[var(--canvas-tan)] p-4">
              <div className="grid gap-2 md:grid-cols-2">
                <p className="text-[16px] font-bold text-[var(--black)]">{item.source_event_name}</p>
                <p className="text-[13px] font-mono uppercase tracking-[0.1em] text-[var(--gray-dark)]">Status: {item.review_state}</p>
                <p className="text-[14px] text-[var(--gray-dark)]">Requirement: {item.requirement || 'Not provided'}</p>
                <p className="text-[14px] text-[var(--gray-dark)]">Knowledge Gap: {item.knowledge_gap || 'Not provided'}</p>
                <p className="text-[14px] text-[var(--gray-dark)]">Evidence Label: {item.evidence_label || 'Not provided'}</p>
                <p className="text-[14px] text-[var(--gray-dark)]">Source Status: {item.source_status}</p>
              </div>
              <div className="mt-3 flex items-center justify-between border-t border-[var(--black)] pt-3">
                <p className="font-mono text-[12px] uppercase tracking-[0.12em] text-[var(--gray-dark)]">Next: Evidence Review</p>
                <Link
                  href="/evidence"
                  className="tactical-btn min-h-[44px] px-3 text-[12px]"
                >
                  Move to Evidence -&gt;
                </Link>
              </div>
        ...
        <section className="space-y-3 border-2 border-[var(--black)] bg-[var(--canvas-tan-light)] p-4">
          <p className="text-[12px] font-mono uppercase tracking-[0.18em] text-[var(--gray-dark)]">Operational Research Requirements</p>
          <form onSubmit={handleCreateRequirement} className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-3">
              <button
                type="submit"
                className="tactical-btn min-h-[44px] px-4 text-[12px]"
              >
                Save Requirement
              </button>
              <p className="text-[12px] text-[var(--gray-dark)]">{submitMessage || 'Persist a requirement from evidence or a manual note.'}</p>
            </div>
          </form>
          <div className="space-y-2">
            {requirements.map((requirement) => (
              <article key={requirement.research_requirement_id} className="border border-[var(--black)] bg-[var(--canvas-tan)] p-3">
                <div className="flex flex-wrap items-center justify-between gap-2">
                  <p className="text-[14px] font-bold text-[var(--black)]">{requirement.source_event_name}</p>
                  <p className="text-[11px] font-mono uppercase tracking-[0.14em] text-[var(--gray-dark)]">{requirement.status}</p>
                </div>
                <p className="mt-2 text-[13px] text-[var(--gray-dark)]">{requirement.research_requirement}</p>
                <p className="mt-1 text-[12px] text-[var(--gray-dark)]">Gap: {requirement.knowledge_gap}</p>
                <p className="mt-1 text-[12px] text-[var(--gray-dark)]">
                ...
                {requirement.status === 'open' ? (
                  <button
                    ...
                    className="mt-3 inline-flex min-h-[44px] items-center border-2 border-[var(--status-ready)] bg-[var(--status-ready)] px-3 text-[12px] font-bold uppercase text-[var(--white)] transition hover:opacity-80 disabled:cursor-not-allowed disabled:opacity-60"
                  >
