    ...
    <RoleStandaloneView roleLabel="Coach Workspace" routeLabel="/coach/progression-intelligence" allowedRoles={['coach']} room="floor" showShellHeader={false}>
      <div className="space-y-6">
        <header className="border-2 border-[var(--patina-700)] bg-[var(--hide-950)] p-5">
          ...
          <h1 className="mt-2 text-3xl font-black text-[var(--bone-100)]">Progression Gaps → Drills → Verification</h1>
        ...
        <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-4">
          <label htmlFor="athlete-input" className="block text-sm font-semibold text-[var(--bone-200)]">Select Athlete</label>
          <input
            id="athlete-input"
            type="text"
            ...
            className="mt-2 w-full border border-[var(--hide-500)] bg-[var(--hide-950)] p-2 text-[var(--bone-200)] placeholder-[var(--bone-400)]"
          />
        </div>
          ...
          <>
            ...
            <section className="space-y-4">
              <div className="flex items-center justify-between">
                <h2 className="text-lg font-bold text-[var(--bone-100)]">Progression Gaps ({gaps.length})</h2>
                <button
              ...
              {showGapForm && (
                <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-4 space-y-3">
                  <div>
                    <label htmlFor="gap-type-select" className="block text-xs font-bold uppercase text-[var(--brass-300)]">Gap Type</label>
                    <select
                      id="gap-type-select"
                  ...
                  <div>
                    <label htmlFor="gap-desc-textarea" className="block text-xs font-bold uppercase text-[var(--brass-300)]">Description</label>
                    <textarea
                  ...
                  <div>
                    <label htmlFor="severity-select" className="block text-xs font-bold uppercase text-[var(--brass-300)]">Severity</label>
                    <select
                      id="severity-select"
              ...
              <div className="space-y-2">
                {gaps.length === 0 ? (
                  ...
                  gaps.map((gap) => (
                    <div key={gap.gap_id} className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <p className="font-semibold text-[var(--bone-200)]">{gap.gap_description}</p>
                          <div className="mt-1 flex gap-2">
                            <span className="text-xs font-mono text-[var(--brass-300)]">{gap.gap_type}</span>
                            <span className="text-xs font-mono text-[var(--brass-300)]">{gap.severity}</span>
            ...
            <section>
              <h2 className="mb-4 text-lg font-bold text-[var(--bone-100)]">Assigned Drills ({assignments.length})</h2>
              <div className="space-y-2">
                {assignments.length === 0 ? (
                  <p className="text-sm text-[var(--bone-400)]">No drills assigned yet.</p>
                ) : (
                  assignments.map((assignment) => (
                    <div key={assignment.assignment_id} className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-3">
                      <div className="flex items-start justify-between gap-2">
                        <div className="flex-1">
                          <p className="font-semibold text-[var(--bone-200)]">{assignment.drill_name}</p>
                          <p className="text-xs text-[var(--bone-300)]">{assignment.drill_description}</p>
                          <div className="mt-2 bg-[var(--hide-950)] h-2 border border-[var(--hide-500)]">
                            <div
