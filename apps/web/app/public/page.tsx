      ...
      <div className="mx-auto flex min-h-screen w-full max-w-7xl flex-col px-6 py-10 lg:px-10">
        <header className="flex flex-col gap-4 border-b-[3px] border-[var(--black)] pb-6 md:flex-row md:items-end md:justify-between">
          <div className="space-y-3">
            <p className="text-xs font-mono uppercase tracking-[0.35em] text-[var(--safety-locked)]">PUBLIC PORTAL</p>
          ...
          <div className="flex flex-wrap items-center gap-2">
            <Link
              href="/login"
              className="inline-flex min-h-[44px] items-center justify-center border-2 border-[var(--black)] bg-[var(--safety-locked)] px-3 text-xs font-mono font-bold uppercase tracking-[0.12em] text-[var(--white)] transition hover:bg-[var(--red-highlight)]"
            >
              Member Login
            </Link>
        ...
        {activeRole === 'admin' && (
          <section className="mt-6 border-[3px] border-[var(--black)] bg-[var(--safety-locked)] p-5 text-[var(--white)] shadow-[var(--shadow-md)]">
            ...
            <div className="mt-3 flex flex-wrap items-center gap-2">
              ...
              <Link
                href="/admin"
                className="inline-flex min-h-[44px] items-center justify-center border-2 border-[var(--white)] bg-transparent px-3 text-xs font-mono font-bold uppercase tracking-[0.12em] text-[var(--white)] transition hover:bg-[var(--white)] hover:text-[var(--safety-locked)]"
              >
        ...
        <section className="mt-6 border-[3px] border-[var(--black)] bg-[var(--canvas-tan-light)] p-5 shadow-[var(--shadow-md)]">
          <p className="text-xs font-mono uppercase tracking-[0.25em] text-[var(--safety-locked)]">Identity</p>
        ...
        <section id="volunteer-recruitment" className="border-[3px] border-[var(--black)] bg-[var(--canvas-tan-light)] p-5 shadow-[var(--shadow-sm)]">
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-3">
            {pathwayCards.map((path) => {
              ...
              return (
                <article key={path.title} className={`border-2 p-4 ${selected ? 'border-[var(--safety-locked)] bg-[var(--canvas-tan-dark)]' : 'border-[var(--black)] bg-[var(--canvas-tan)]'}`}>
                  ...
                  <button
                    ...
                    className="mt-3 min-h-[44px] border-2 border-[var(--black)] bg-[var(--safety-locked)] px-3 text-[14px] font-bold text-[var(--white)] transition hover:bg-[var(--red-highlight)]"
                  >
                    Select
                  </button>
                </article>
        ...
        <section id="interest-intake" className="border-[3px] border-[var(--black)] bg-[var(--canvas-tan-light)] p-5 shadow-[var(--shadow-sm)]">
          ...
          <form className="mt-4 grid gap-3" onSubmit={handleSubmit}>
            ...
            <button
              type="submit"
              disabled={submitting}
              className="min-h-[44px] border-2 border-[var(--black)] bg-[var(--safety-locked)] px-4 text-[14px] font-bold text-[var(--white)] transition hover:bg-[var(--red-highlight)] disabled:cursor-not-allowed disabled:opacity-60"
            >
              {submitting ? 'Submitting...' : 'Submit Interest'}
            </button>

            {confirmation && <p className="text-[14px] text-[var(--safety-locked)]">{confirmation}</p>}
          </form>
        </section>
              ...
              return (
                <article key={item.question} className="border-2 border-[var(--black)] bg-[var(--canvas-tan)]">
                  ...
                  >
                    ...
                    <span className="font-mono text-[var(--safety-locked)]">{expanded ? '−' : '+'}</span>
                  </button>
                  {expanded && (
                    <p
        ...
        <section className="border-[3px] border-[var(--black)] bg-[var(--canvas-tan-light)] p-5 shadow-[var(--shadow-sm)]">
          ...
          <div className="mt-3 grid gap-2 text-[14px] font-mono text-[var(--safety-locked)]">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            ...
            {[
              ...
              <a
                ...
                className="inline-flex min-h-[44px] items-center border-2 border-[var(--black)] bg-[var(--safety-locked)] px-4 text-[14px] font-bold text-[var(--white)] transition hover:bg-[var(--red-highlight)]"
              >
                {link.label}
              </a>
            ))}
          ...
          <div className="mt-3 max-h-[220px] space-y-2 overflow-y-auto border-2 border-[var(--black)] bg-[var(--canvas-tan)] p-3">
            ...
            {telemetryTraces.map((trace, idx) => (
              <div key={`${trace.timestamp}-${idx}`} className="border-2 border-[var(--black)] bg-[var(--canvas-tan-light)] p-2 text-[13px] text-[var(--gray-dark)]">
                <p className="font-mono text-[var(--safety-locked)]">[{trace.timestamp}]</p>
                <p>{trace.event}</p>
                <p>{trace.detail}</p>
              </div>
            ))}
