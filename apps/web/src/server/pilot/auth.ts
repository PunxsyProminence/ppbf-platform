...
// Exported so legacyLocalAccountFixtures.ts (the deprecated-account test
// fixtures moved out of this file) can reuse them without duplicating the SQL.
export async function revokeAllSessionsForAccountTx(client: PoolClient, accountId: string): Promise<void> {
  await client.query(
...
export async function assignOrganizationMembershipTx(
  client: PoolClient,
  accountId: string,
  organizationId: string,
  role: PilotRole,
