...
const MIGRATION_RUNNER_PATH = path.resolve(
  __dirname,
  '../../../scripts/pilot-apply-gear-migration.mjs',
);
...
// schema and proves nothing about scripts/pilot-apply-gear-migration.mjs's
...
describe('gear runner readiness assertion', () => {
  ...
  test('the real runner REFUSES a database where the migration never ran', async () => {
    ...
    const client = await runnerDatabase('ppbf_test_gear_rdy_no');
    try {
  ...
  test('the real runner ACCEPTS a correctly migrated database, and a re-apply stays a no-op', async () => {
    const applyMigrationTransaction = await loadRunner();
    const client = await runnerDatabase('ppbf_test_gear_rdy_ok');
    try {
      const migrationSql = await fs.readFile(path.join(INFRA_DIR, 'pilot_slice_postgres_gear_migration.sql'), 'utf8');
