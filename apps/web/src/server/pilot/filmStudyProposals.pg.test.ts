...
const MIGRATION_RUNNER_PATH = path.resolve(
  __dirname,
  '../../../scripts/pilot-apply-film-study-proposals-migration.mjs',
);
...
// schema and proves nothing about scripts/pilot-apply-film-study-proposals-migration.mjs's
...
describe('film study proposals runner readiness assertion', () => {
  ...
  test('the real runner REFUSES a database where the migration never ran', async () => {
    ...
    const client = await runnerDatabase('ppbf_test_fsprop_rdy_no');
    try {
      await expect(applyMigrationTransaction(client, 'select 1')).rejects.toThrow(
        /FILM_STUDY_PROPOSALS_TABLE_NOT_READY/,
      );
    } finally {
      await client.end();
    }
  ...
  test('the real runner ACCEPTS a correctly migrated database, and a re-apply stays a no-op', async () => {
    const applyMigrationTransaction = await loadRunner();
    const client = await runnerDatabase('ppbf_test_fsprop_rdy_ok');
    try {
      const migrationSql = await fs.readFile(path.join(INFRA_DIR, 'pilot_slice_postgres_film_study_proposals_migration.sql'), 'utf8');
