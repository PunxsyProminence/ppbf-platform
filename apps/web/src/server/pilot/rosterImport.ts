...
/**
 * pilot.athletes.gym_status is plain text in the database, so the vocabulary is
 * held together in the application. This used to be a fourth private copy of the
 * list; it now reads the shared one, so a spreadsheet inventing a spelling is
 * refused by the same allow-list the roster form offers, the athlete validator
 * enforces, and intake promotion checks.
 */
const GYM_STATUSES = new Set<string>(GYM_STATUS_OPTIONS);
  ...
  for (const [index, row] of rows.entries()) {
    ...
    try {
      const created = await insertAthleteIfAbsent(organizationId, {
        ...
        //
        // Narrowed with the shared guard rather than cast. rejectionReason has
        // already refused any non-empty value outside the vocabulary, so the
        // fallback is only ever reached by a blank cell -- and if that ever
        // stopped being true, this writes the safe default instead of a value
        // the column's readers would not recognise.
        gym_status: isGymStatus(row.gym_status) ? row.gym_status : 'training',
