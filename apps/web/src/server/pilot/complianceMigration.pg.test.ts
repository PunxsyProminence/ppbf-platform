...
const MIGRATION_RUNNER_PATH = path.resolve(
  __dirname,
  '../../../scripts/pilot-apply-compliance-migration.mjs',
);
...
// scripts/pilot-apply-compliance-migration.mjs's READINESS_QUERY -- the
...
describe('compliance runner readiness assertion', () => {
  test('the real runner REFUSES a database where the migration never ran', async () => {
    const client = await freshDatabase('complmig_rdy_no');
    try {
      await expect(applyMigrationTransaction(client, 'select 1')).rejects.toThrow(
        /COMPLIANCE_TABLES_NOT_READY/,
      );
    } finally {
      await client.end();
    }
  ...
  test('the real runner ACCEPTS a correctly migrated database, and a re-apply stays a no-op', async () => {
    const client = await freshDatabase('complmig_rdy_ok');
    try {
