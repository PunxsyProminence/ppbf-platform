...
describe('resetAccountPin', () => {
  test('changes the PIN and revokes sessions in the same transaction on success', async () => {
    ...
    await resetAccountPin('acct-1', '482913', 'org-1');
  ...
  test('never revokes sessions when the account is not found or not owned by this organization', async () => {
    ...
    await expect(resetAccountPin('acct-1', '482913', 'org-other')).rejects.toThrow(
  ...
  test('rejects reset for non-athlete accounts', async () => {
    ...
    await expect(resetAccountPin('coach-1', '482913', 'org-1')).rejects.toThrow(
      'Not found: no such account, or it cannot be reset',
    );
  ...
  test('the refusal reaches the admin as a 404, not a masked 500', async () => {
    ...
    const refusal = await resetAccountPin('acct-1', '482913', 'org-other').catch((error) => error);
  ...
  test('does not report success when the revoke step fails after the PIN update', async () => {
    ...
    await expect(resetAccountPin('acct-1', '482913', 'org-1')).rejects.toThrow('connection lost');
  });
...
describe('activateAccountPin', () => {
  ...
  test('rejects activation when account is outside organization scope', async () => {
    ...
    await expect(activateAccountPin('ath-acct-1', '482913', 'org-2')).rejects.toThrow(
