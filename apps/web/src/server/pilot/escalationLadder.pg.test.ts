...
const MIGRATION_RUNNER_PATH = path.resolve(
  __dirname,
  '../../../scripts/pilot-apply-safety-escalations-migration.mjs',
);
...
// scripts/pilot-apply-safety-escalations-migration.mjs's READINESS_QUERY -- the
...
describe('safety escalations runner readiness assertion', () => {
  // The `dropEscalationsTableFirst: true` is load-bearing, not tidiness.
  // pilot_slice_postgres.sql already ships pilot.safety_escalations, so against a
  ...
  test('the real runner REFUSES a database where the migration never ran', async () => {
    const client = await freshDatabase('ppbf_test_escal_rdy_no', { dropEscalationsTableFirst: true });
    try {
      await expect(applyMigrationTransaction(client, 'select 1')).rejects.toThrow(
        /SAFETY_ESCALATIONS_NOT_READY/,
      );
    } finally {
      await client.end();
    }
  ...
  test('the real runner ACCEPTS a correctly migrated database, and a re-apply stays a no-op', async () => {
    const client = await freshDatabase('ppbf_test_escal_rdy_ok', { dropEscalationsTableFirst: true });
    try {
