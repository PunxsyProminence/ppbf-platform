...
const MIGRATION_RUNNER_PATH = path.resolve(
  __dirname,
  '../../../scripts/pilot-apply-emergency-contact-note-migration.mjs',
);
...
// scripts/pilot-apply-emergency-contact-note-migration.mjs's READINESS_QUERY -- the
...
describe('emergency contact note runner readiness assertion', () => {
  test('the real runner REFUSES a database where the migration never ran', async () => {
    const client = await freshDatabase('emercn_rdy_no');
    try {
      await expect(applyMigrationTransaction(client, 'select 1')).rejects.toThrow(
        /EMERGENCY_CONTACT_NOTE_NOT_READY/,
      );
    } finally {
      await client.end();
    }
  ...
  test('the real runner ACCEPTS a correctly migrated database, and a re-apply stays a no-op', async () => {
    const client = await freshDatabase('emercn_rdy_ok');
    try {
