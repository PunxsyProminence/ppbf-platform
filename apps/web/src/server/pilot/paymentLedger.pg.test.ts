...
const MIGRATION_RUNNER_PATH = path.resolve(
  __dirname,
  '../../../scripts/pilot-apply-payments-migration.mjs',
);
...
// scripts/pilot-apply-payments-migration.mjs's READINESS_QUERY -- the
...
describe('payments runner readiness assertion', () => {
  test('the real runner REFUSES a database where the migration never ran', async () => {
    const client = await freshDatabase('payled_rdy_no');
    try {
  ...
  test('the real runner ACCEPTS a correctly migrated database, and a re-apply stays a no-op', async () => {
    const client = await freshDatabase('payled_rdy_ok');
    try {
