...
const MIGRATION_RUNNER_PATH = path.resolve(
  __dirname,
  '../../../scripts/pilot-apply-goal-category-progress-migration.mjs',
);
...
// scripts/pilot-apply-goal-category-progress-migration.mjs's READINESS_QUERY -- the
...
describe('goal category progress runner readiness assertion', () => {
  test('the real runner REFUSES a database where the migration never ran', async () => {
    const client = await freshDatabase('goalcp_rdy_no');
    try {
      await expect(applyMigrationTransaction(client, 'select 1')).rejects.toThrow(
        /GOAL_CATEGORY_PROGRESS_NOT_READY/,
      );
    } finally {
      await client.end();
    }
  ...
  test('the real runner ACCEPTS a correctly migrated database, and a re-apply stays a no-op', async () => {
    const client = await freshDatabase('goalcp_rdy_ok');
    try {
