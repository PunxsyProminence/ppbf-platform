...
// Applies the programs-catalog migration inside one transaction, with the
...
const READINESS_QUERY = `
  select
    ...
    ) as name_nonblank_ready
`;

function assertReadiness(row) {
  if (!row || Object.values(row).some((value) => value !== true)) {
    throw new Error('PROGRAMS_NOT_READY');
  }
}
...
export async function run() {
  ...
  const migrationPath = path.resolve(
    __dirname,
    '../../../infra/azure/pilot_slice_postgres_programs_migration.sql',
  );
  ...
  console.log(`Applied programs migration: ${migrationPath}`);
  console.log('PILOT PROGRAMS MIGRATION PASS');
}
...
if (isMainModule) {
  try {
    ...
    console.error('PILOT PROGRAMS MIGRATION FAIL');
