...
// Applies the calibration-annotations migration inside one transaction, with the same
...
// run. The two TRIGGERS are asserted explicitly: they are what makes a
// submitted annotation set immutable, and therefore what makes "independent"
// a property of the system rather than a claim about intent. A schema that
// has the tables but not the triggers looks ready and silently is not.
const READINESS_QUERY = `
  select
    to_regclass('pilot.calibration_annotation_sets') is not null as sets_table_ready,
    to_regclass('pilot.calibration_annotation_events') is not null as events_table_ready,
    exists (
    ...
    exists (
      select 1 from pg_constraint
      where conrelid = to_regclass('pilot.calibration_annotation_sets')
        and conname = 'pilot_calibration_sets_submission_attested'
    ) as submission_attested_ready,
    exists (
      select 1 from pg_constraint
      where conrelid = to_regclass('pilot.calibration_annotation_events')
        and conname = 'pilot_calibration_events_class_shape'
    ) as class_shape_ready,
    exists (
      select 1 from pg_constraint
      where conrelid = to_regclass('pilot.calibration_annotation_events')
        and conname = 'pilot_calibration_events_within_clip'
    ) as containment_check_ready,
    exists (
      select 1 from pg_constraint
      where conrelid = to_regclass('pilot.calibration_annotation_events')
        and conname = 'pilot_calibration_events_clip_fk'
    ) as containment_fk_ready,
    exists (
      select 1 from pg_constraint
      where conrelid = to_regclass('pilot.calibration_annotation_events')
        and conname = 'pilot_calibration_events_counter_fk'
    ) as counter_scoping_ready,
    exists (
      select 1 from pg_constraint
      where conrelid = to_regclass('pilot.calibration_annotation_events')
        and conname = 'pilot_calibration_events_defends_fk'
    ) as defends_scoping_ready,
    exists (
      select 1 from pg_trigger
      where tgrelid = to_regclass('pilot.calibration_annotation_sets')
        and tgname = 'pilot_calibration_sets_freeze'
    ) as set_freeze_ready,
    exists (
      select 1 from pg_trigger
      where tgrelid = to_regclass('pilot.calibration_annotation_events')
        and tgname = 'pilot_calibration_events_freeze'
    ) as event_freeze_ready
`;

function assertReadiness(row) {
  if (!row || Object.values(row).some((value) => value !== true)) {
    throw new Error('CALIBRATION_ANNOTATIONS_NOT_READY');
  }
}
...
export async function run() {
  ...
  const migrationPath = path.resolve(
    __dirname,
    '../../../infra/azure/pilot_slice_postgres_calibration_annotations_migration.sql',
  );
  ...
  console.log(`Applied calibration annotations migration: ${migrationPath}`);
  console.log('PILOT CALIBRATION ANNOTATIONS MIGRATION PASS');
}
...
if (isMainModule) {
  try {
    ...
    console.error('PILOT CALIBRATION ANNOTATIONS MIGRATION FAIL');
