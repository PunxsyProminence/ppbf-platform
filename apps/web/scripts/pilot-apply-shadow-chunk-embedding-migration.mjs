// Applies the SHADOW chunk embedding migration.
//
// Unreachable until 2026-08-01. shadowLibrary.ts writes embeddings to these
// columns and filters retrieval on `c.embedding is not null`; without them
// library retrieval degrades silently rather than failing loudly.
//
...
const READINESS_QUERY = `
  select
    ...
    ) as library_chunks_columns_ready
`;

function assertReadiness(row) {
  if (!row || Object.values(row).some((value) => value !== true)) {
    throw new Error('SHADOW_CHUNK_EMBEDDING_NOT_READY');
  }
}
...
export async function run() {
  ...
  const migrationPath = path.resolve(
    __dirname,
    '../../../infra/azure/pilot_slice_postgres_shadow_chunk_embedding_migration.sql',
  );
  ...
  console.log(`Applied SHADOW chunk embedding migration: ${migrationPath}`);
  console.log('PILOT SHADOW CHUNK EMBEDDING MIGRATION PASS');
}
...
if (isMainModule) {
  try {
    ...
    console.error('PILOT SHADOW CHUNK EMBEDDING MIGRATION FAIL');
