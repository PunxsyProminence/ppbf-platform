...
// is asserted where it belongs: cohortDefinitionsDisciplineFk.pg.test.ts.
//
...
const READINESS_QUERY = `
  select
    ...
    to_regclass('pilot.cohort_definitions') is not null as cohortdef_table_ready,
    exists (
...
function assertReadiness(row) {
  if (!row || Object.values(row).some((value) => value !== true)) {
    throw new Error('COHORT_DEFINITIONS_DISCIPLINE_FK_NOT_READY');
  }
}
...
export async function run() {
  ...
  const migrationPath = path.resolve(
    __dirname,
    '../../../infra/azure/pilot_slice_postgres_cohort_definitions_discipline_fk_migration.sql',
  );
  ...
  console.log(`Applied cohort definitions discipline FK migration: ${migrationPath}`);
  console.log('PILOT COHORT DEFINITIONS DISCIPLINE FK MIGRATION PASS');
}
...
if (isMainModule) {
  try {
    ...
    console.error('PILOT COHORT DEFINITIONS DISCIPLINE FK MIGRATION FAIL');
