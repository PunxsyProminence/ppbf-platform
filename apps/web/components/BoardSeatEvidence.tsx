...
export default function BoardSeatEvidence({ seat }: Readonly<{ seat: BoardSeatSlug }>) {
  ...
  if (failed) {
    return (
      <article className="border-2 border-[var(--black)] bg-[var(--canvas-tan-light)] p-5">
        <h2 className="text-xl font-black text-[var(--black)]">Records could not be read</h2>
  ...
  if (wantsRules) {
    ...
    return (
      <article className="border-2 border-[var(--black)] bg-[var(--canvas-tan-light)] p-5">
        <h2 className="text-xl font-black text-[var(--black)]">The standard this gym holds itself to</h2>
        <p className="mt-2 text-sm leading-6 text-[var(--gray-dark)]">
        ...
        {ordered.length === 0 ? (
          <p className="mt-4 text-sm leading-6 text-[var(--gray-dark)]">
            ...
            {ordered.map((rule) => (
              <li key={rule.rule_id} className="border-2 border-[var(--black)] bg-[var(--canvas-tan)] p-3">
                <div className="flex flex-wrap items-baseline justify-between gap-2">
                  ...
                  <p className="text-[12px] font-mono uppercase tracking-[0.12em] text-[var(--black)]">
                    {rule.severity}
                  </p>
                </div>
                <p className="mt-1 text-[13px] leading-5 text-[var(--gray-dark)]">
  ...
  return (
    <article className="border-2 border-[var(--black)] bg-[var(--canvas-tan-light)] p-5">
      <h2 className="text-xl font-black text-[var(--black)]">Board communications register</h2>
      <p className="mt-2 text-sm leading-6 text-[var(--gray-dark)]">
      ...
      {notices.length === 0 ? (
        <p className="mt-4 text-sm leading-6 text-[var(--gray-dark)]">No notices have been published.</p>
      ) : (
        <ul className="mt-4 space-y-3">
          {notices.map((notice) => (
            <li key={notice.announcement_id} className="border-2 border-[var(--black)] bg-[var(--canvas-tan)] p-3">
              <p className="text-sm leading-6 text-[var(--black)]">{notice.message}</p>
              <p className="mt-2 text-[12px] font-mono uppercase tracking-[0.12em] text-[var(--gray-dark)]">
