...
function taskStatusTone(status: CoachTask['status']): string {
  if (status === 'Open') return 'bg-[var(--hide-600)] text-[var(--brass-300)]';
  if (status === 'In Progress') return 'bg-[var(--cleared)] text-[var(--cleared-ink)]';
  return 'bg-[var(--hide-700)] text-[var(--monitor-ink)]';
}
    ...
    <div className="min-h-screen bg-[var(--hide-950)] text-[var(--bone-200)] font-sans">
      <div className="max-w-7xl mx-auto p-4 space-y-8">
        ...
        <div className="border-b-2 border-[var(--patina-700)] pb-6 space-y-4">
          <div>
            <p className="text-xs font-mono uppercase tracking-[0.15em] text-[var(--brass-300)]">Coach Development Workspace</p>
            <h1 className="text-3xl md:text-4xl font-black mt-2">Live Session Management</h1>
            <p className="text-base text-[var(--bone-400)] mt-2">Manage your program floor, develop yourself, and track athlete progress with SMART goals and assessments.</p>
        ...
        <div className="border border-[var(--hide-600)] bg-[var(--hide-950)] p-4">
          <p className="text-sm text-[var(--brass-300)] font-semibold">Coach Standard</p>
          <p className="mt-1 text-sm text-[var(--bone-300)]">Lead with discipline, protect the culture, and model the grind. The room rises when the coach stays locked in.</p>
        </div>
        ...
        {/* MODE TOGGLE */}
        <div className="flex w-fit gap-2 border-2 border-[var(--patina-700)] bg-[var(--hide-950)] p-2">
        ...
        <div className="space-y-6">
          ...
          {activeTab === 'dashboard' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <section className="grid gap-3 md:grid-cols-3">
                <article className="border border-[var(--hide-500)] bg-[var(--hide-950)] px-4 py-3">
                  <p className="text-xs font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">Readiness Alerts</p>
                  {readinessTrackingAvailable ? (
                    <>
                      <p className="mt-2 text-2xl font-black text-[var(--bone-100)]">{redReadinessCount + yellowReadinessCount}</p>
                      <p className="text-xs text-[var(--bone-400)]">{redReadinessCount} RED, {yellowReadinessCount} YELLOW</p>
                    </>
                  ) : (
                    <>
                      <p className="mt-2 text-2xl font-black text-[var(--bone-400)]">Not tracked</p>
                      <p className="text-xs text-[var(--bone-400)]">No backend readiness feed yet -- do not read this as &quot;zero flags&quot;</p>
                    </>
                  )}
                </article>
                <article className="border border-[var(--hide-500)] bg-[var(--hide-950)] px-4 py-3">
                  <p className="text-xs font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">Injury Flags</p>
                  {injuryTrackingAvailable ? (
                    <>
                      <p className="mt-2 text-2xl font-black text-[var(--bone-100)]">{injuryFlags}</p>
                      <p className="text-xs text-[var(--bone-400)]">Escalate before block progression</p>
                    </>
                  ) : (
                    <>
                      <p className="mt-2 text-2xl font-black text-[var(--bone-400)]">Not tracked</p>
                      <p className="text-xs text-[var(--bone-400)]">No backend injury feed yet -- do not read this as &quot;no injuries&quot;</p>
                    </>
                  )}
                </article>
                <article className="border border-[var(--hide-500)] bg-[var(--hide-950)] px-4 py-3">
                  <p className="text-xs font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">Open Reviews</p>
                  <p className="mt-2 text-2xl font-black text-[var(--bone-100)]">{reviewsNeeded}</p>
                  <p className="text-xs text-[var(--bone-400)]">Resolve queue items this session</p>
                </article>
              </section>
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div className={ui.panelSpaced}>
                  <h3 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">Today&apos;s Session</h3>
                  ...
                  <div className="space-y-3">
                    <div>
                      <p className="text-xs text-[var(--bone-400)] block mb-1">Session Name</p>
                      <p className="text-base font-semibold text-[var(--bone-400)]">Unavailable - not yet tracked</p>
                    </div>
                    <div>
                      <p className="text-xs text-[var(--bone-400)] block mb-1">Time</p>
                      <p className="text-base font-semibold text-[var(--bone-400)]">Unavailable - not yet tracked</p>
                    </div>
                    <div>
                      <p className="text-xs text-[var(--bone-400)] block mb-1">Status</p>
                      <p className="text-base font-semibold text-[var(--bone-400)]">Unavailable - not yet tracked</p>
                    </div>
                    <div>
                      <p className="text-xs text-[var(--bone-400)] block mb-1">Athletes Present</p>
                      <p className="text-base font-semibold">
                        {trackedAttendanceCount > 0 ? `${activeAthletes}/${athletes.length}` : (
                          <span className="text-[var(--bone-400)]">Not tracked</span>
                        )}
                      </p>
                    </div>
                  </div>
                ...
                {/* Athlete Roster */}
                <div className={ui.panelSpaced}>
                  <h3 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">Athlete Roster</h3>
                  
                  {athletesLoading && (
                    <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-950)] p-4 text-center">
                      <p className="text-[var(--bone-400)] text-sm">Loading athletes...</p>
                  ...
                  {!athletesLoading && athletes.length === 0 && !athletesError && (
                    <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-950)] p-4 text-center">
                      <p className="text-[var(--bone-400)] text-sm">No athletes found</p>
                    </div>
                  )}
                      ...
                      <button
                        ...
                        className={`w-full p-3 border-2 rounded cursor-pointer transition text-left ${
                          selectedAthleteId === athlete.id
                            ? 'bg-[var(--hide-800)] border-[var(--patina-700)]'
                            : 'bg-[var(--hide-950)] border-[var(--hide-600)] hover:border-[var(--patina-700)]'
                        }`}
                      >
                        <div className="flex items-center justify-between">
                          ...
                          <span className="text-xs text-[var(--bone-400)]">{athlete.attendance}</span>
                        </div>
                        {athlete.injuryFlag && (
                ...
                {/* Open Tasks */}
                <div className="md:col-span-2 border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-6 space-y-4">
                  <h3 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">Open Tasks</h3>
                  <div className="space-y-2 max-h-48 overflow-y-auto">
                    {coachTasks.filter(t => t.status !== 'Completed').map(task => (
                      <div key={task.id} className="border-2 border-[var(--patina-700)] bg-[var(--hide-950)] p-3">
                        ...
                        <p className="text-xs text-[var(--bone-400)]">{task.when}</p>
                      </div>
                    ))}
                    {coachTasks.length === 0 && (
                      <p className="text-xs text-[var(--bone-400)]">No open tasks. Items appear here from the SHADOW review queue.</p>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              {floorPlansError ? (
                ...
                <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-6">
                  <p className="text-sm text-[var(--brass-300)] font-semibold">No athlete floor plans received yet.</p>
                  <p className="mt-2 text-sm text-[var(--bone-400)]">Once an athlete checks in and their floor plan auto-generates, it will appear here as an individual coach review tab.</p>
                </div>
              ) : (
                <div className="space-y-4">
                  {athleteFloorPlans.map((plan, index) => (
                    <article key={`${plan.athleteName}-${plan.generatedAt}-${index}`} className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-5 space-y-4">
                      <div className="flex flex-wrap items-center justify-between gap-2">
                        <div>
                          <p className="text-xs font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">Individual Plan</p>
                          <h4 className="text-lg font-semibold text-[var(--bone-200)]">{plan.athleteName}</h4>
                          <p className="text-xs text-[var(--bone-400)]">Generated {new Date(plan.generatedAt).toLocaleString()}</p>
                        </div>
                      ...
                      <div className="grid gap-3 md:grid-cols-2">
                        {plan.tasks.map((task) => (
                          <div key={task.id} className="border border-[var(--hide-600)] bg-[var(--hide-950)] p-3">
                            <div className="flex items-center justify-between gap-2">
                              <p className="text-sm font-semibold text-[var(--bone-200)]">{task.title}</p>
                            ...
                            <p className="mt-1 text-xs text-[var(--brass-300)]">{task.category} - {task.dueDate}</p>
                            <p className="mt-2 text-xs text-[var(--bone-400)]">{task.description}</p>
                          </div>
                        ))}
                      </div>
                    </article>
          ...
          {activeTab === 'floor' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-6 space-y-4">
                <h3 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">Session Workout Plan</h3>
                <p className="font-mono text-xs font-bold uppercase tracking-[0.1em] text-[var(--locked)]">
                  PLANNED | NOT YET IMPLEMENTED
                </p>
                <p className="text-xs text-[var(--bone-400)]">
                ...
                <div className="space-y-2">
                  {workoutBlocks.map((block) => (
                    <div key={block.id} className="border-2 border-[var(--patina-700)] bg-[var(--hide-950)] p-3 rounded">
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="font-semibold">{block.title}</p>
                          <p className="text-xs text-[var(--bone-400)]">{block.duration} minutes</p>
                          <p className="text-xs text-[var(--brass-300)] mt-1">{block.objective}</p>
                        </div>
                      </div>
                      <div className="mt-3 grid gap-2 md:grid-cols-2">
                        <div className="border border-[var(--hide-600)] bg-[var(--hide-950)] p-2">
                          <p className="text-[11px] font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">Planned Training</p>
                          <ul className="mt-1 space-y-1 text-xs text-[var(--bone-300)]">
                        ...
                        <div className="border border-[var(--hide-600)] bg-[var(--hide-950)] p-2">
                          <p className="text-[11px] font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">Coach Cues</p>
                          <ul className="mt-1 space-y-1 text-xs text-[var(--bone-300)]">
          ...
          {activeTab === 'development' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-6 space-y-4">
                  <h3 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">Current Certifications</h3>
                  <p className="font-mono text-xs font-bold uppercase tracking-[0.1em] text-[var(--locked)]">
                    PLANNED | NOT YET IMPLEMENTED
                  </p>
                ...
                <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-6 space-y-4">
                  <h3 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">Development Topics</h3>
                  <p className="font-mono text-xs font-bold uppercase tracking-[0.1em] text-[var(--locked)]">
                    PLANNED | NOT YET IMPLEMENTED
                  </p>
                  <p className="text-sm text-[var(--bone-400)]">
                  ...
                  <ul className="space-y-2">
                    {[
                      ...
                      <li key={topic} className="p-2 border-2 border-[var(--patina-700)] bg-[var(--hide-950)] text-sm">
                        {topic}
                      </li>
                    ))}
                  </ul>
                ...
                {coachGoals.map(goal => (
                  <div key={goal.id} className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-4 space-y-3">
                    <div className="flex justify-between items-start">
                      ...
                      <span className="text-xs bg-[var(--hide-600)] text-[var(--bone-400)] px-2 py-1">{goal.category}</span>
                    </div>
                    <div>
                    ...
                    <p className="text-xs text-[var(--bone-400)]">Due: {goal.dueDate}</p>
                  </div>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="space-y-3">
                {coachTasks.length === 0 && !shadowQueueUnavailable && (
                  <p className="text-sm text-[var(--bone-400)]">No open tasks. Items appear here from the SHADOW review queue.</p>
                )}
                {coachTasks.map(task => (
                  <div key={task.id} className={`border-2 p-4 rounded ${
                    task.status === 'Completed' ? 'bg-[var(--cleared)]/30 border-green-700' : 'bg-[var(--hide-900)] border-[var(--patina-700)]'
                  }`}>
                    <div className="flex justify-between items-start mb-2">
                      <div>
                        <h4 className="font-semibold">{task.title}</h4>
                        <p className="text-xs text-[var(--bone-400)] mt-1">{task.when}</p>
                      </div>
                    ...
                    {task.relatedAthlete && (
                      <p className="text-xs text-[var(--bone-400)]">Related: {athletes.find(a => a.id === task.relatedAthlete)?.name}</p>
                    )}
                  </div>
                ))}
              </div>
          ...
          {activeTab === 'shadow' && (
            <div className="space-y-6 animate-fadeIn">
              ...
              <div className="border-2 border-[var(--brass-500)] bg-[var(--hide-950)] p-6 space-y-4">
                <h3 className="font-mono text-sm font-bold uppercase text-[var(--brass-300)]">SHADOW Coach Assistant</h3>
                <p className="text-sm text-[var(--bone-400)]">Ask questions about session management, athlete readiness, goals, tasks, or coaching strategy.</p>
              </div>

              <div className="grid gap-4 md:grid-cols-2">
                <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-4">
                  <h3 className="font-mono text-xs font-bold uppercase tracking-[0.08em] text-[var(--brass-300)]">SHADOW Review Projection</h3>
                  {shadowQueue.length === 0 ? (
                    <p className="mt-2 text-xs text-[var(--bone-400)]">No SHADOW queue items returned.</p>
                  ) : (
                    <div className="mt-2 space-y-2">
                      {shadowQueue.slice(0, 6).map((item) => (
                        <div key={item.intake_case_id} className="border border-[var(--hide-500)] bg-[var(--hide-950)] p-2 text-xs text-[var(--bone-300)]">
                          <p className="font-semibold text-[var(--bone-200)]">{item.summary}</p>
                ...
                <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-4">
                  ...
                  {shadowObservations.length === 0 ? (
                    <p className="mt-2 text-xs text-[var(--bone-400)]">No SHADOW observation items returned.</p>
                  ) : (
                    <div className="mt-2 space-y-2">
          ...
          {activeTab === 'assessments' && (
            <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-6 space-y-4 animate-fadeIn">
              <h3 className="font-mono font-bold text-[var(--brass-300)] uppercase">Coach Assessments</h3>
              <p className="text-[var(--bone-400)]">Evaluate coaching effectiveness, communication, and athlete development.</p>
              <div className="text-sm text-[var(--bone-400)]">Coming soon: Leadership assessment, communication effectiveness survey, teaching impact evaluation.</div>
            </div>
          )}
          ...
          {activeTab === 'film-study' && (
            <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-6 space-y-4 animate-fadeIn">
              <h3 className="font-mono font-bold text-[var(--brass-300)] uppercase">Film Study</h3>
              <p className="text-[var(--bone-400)]">Record observations from training videos and self-evaluations.</p>
              <div className="text-sm text-[var(--bone-400)]">Coming soon: Video upload, timestamp annotations, technical analysis tools.</div>
              <div className="border border-[var(--hide-500)] bg-[var(--hide-950)] p-3">
                <p className="text-xs font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">AI Video Analysis - Planned</p>
                <p className="mt-1 text-xs text-[var(--bone-300)]">Video Upload: FRONT-END PLACEHOLDER | Skill Recognition: BACKEND REQUIRED | Technique Scoring: ML REQUIRED</p>
                <div className="mt-2 flex flex-wrap gap-2">
                  <Link href="/coach/video-analysis" className="border border-[var(--patina-700)] bg-[var(--rust-900)] px-3 py-1 text-[11px] font-mono uppercase tracking-[0.08em] text-[var(--bone-200)]">
                    Open Video Analysis Surface
                  </Link>
                  <Link href="/athlete/video-analysis" className="border border-[var(--hide-600)] bg-[var(--hide-900)] px-3 py-1 text-[11px] font-mono uppercase tracking-[0.08em] text-[var(--bone-300)]">
          ...
          {activeTab === 'athlete-reviews' && (
            <div className="border-2 border-[var(--patina-700)] bg-[var(--hide-900)] p-6 space-y-4 animate-fadeIn">
              <h3 className="font-mono font-bold text-[var(--brass-300)] uppercase">Athlete Performance Reviews</h3>
              <p className="text-[var(--bone-400)]">Comprehensive athlete progress tracking and performance feedback.</p>
              <div className="border border-[var(--hide-500)] bg-[var(--hide-950)] p-3 space-y-3">
                <p className="text-xs font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">Persist Coach Review</p>
                <input
                  value={reviewSessionId}
                  onChange={(event) => setReviewSessionId(event.target.value)}
                  placeholder="Session ID (from persisted session)"
                  className="h-11 w-full border border-[var(--patina-700)] bg-[var(--hide-900)] px-3 text-sm text-[var(--bone-200)]"
                />
                <select
                ...
                <textarea
                  ...
                  className="min-h-[84px] w-full border border-[var(--patina-700)] bg-[var(--hide-900)] px-3 py-2 text-sm text-[var(--bone-200)]"
                />
                <button
                  type="button"
                  onClick={() => void submitCoachReview()}
                  disabled={reviewSubmitting}
                  className="h-11 border-2 border-[var(--patina-700)] bg-[var(--rust-900)] px-4 text-xs font-mono font-bold uppercase tracking-[0.08em] text-[var(--bone-200)] transition hover:border-[var(--brass-500)] disabled:cursor-not-allowed disabled:opacity-60"
                >
                  {reviewSubmitting ? 'Saving...' : 'Save Coach Review'}
                </button>
                {reviewSyncMessage ? <p className="text-xs text-[var(--brass-300)]">{reviewSyncMessage}</p> : null}
              </div>
              <div className="border border-[var(--hide-500)] bg-[var(--hide-950)] p-3">
                <p className="text-xs font-mono uppercase tracking-[0.08em] text-[var(--brass-300)]">Closed-Loop Progression Intelligence - Planned</p>
                <p className="mt-1 text-xs text-[var(--bone-300)]">Development Recommendation: PLACEHOLDER | Coach Review Required | Human Review Required</p>
                <Link href="/coach/progression-intelligence" className="mt-2 inline-flex border border-[var(--patina-700)] bg-[var(--rust-900)] px-3 py-1 text-[11px] font-mono uppercase tracking-[0.08em] text-[var(--bone-200)]">
