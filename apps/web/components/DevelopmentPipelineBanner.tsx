...
export default function DevelopmentPipelineBanner({ currentStage }: DevelopmentPipelineBannerProps) {
  ...
  return (
    <section className="border-2 border-[var(--patina-700)] bg-[var(--hide-950)] p-4">
      <p className="text-[11px] font-mono uppercase tracking-[0.22em] text-[var(--brass-300)]">PPBF Knowledge Development Pipeline</p>
            ...
            <div key={stage.key} className="flex items-center gap-2">
              <Link
                ...
                className={`inline-flex min-h-[44px] items-center border px-3 py-2 text-[11px] font-mono uppercase tracking-[0.09em] transition ${
                  isCurrent
                    ? 'border-[var(--brass-500)] bg-[var(--rust-700)] text-[var(--bone-100)]'
                    : isComplete
                      ? 'border-[var(--patina-700)] bg-[var(--rust-900)] text-[var(--locked-ink)]'
                      : isUpcoming
                        ? 'border-[var(--hide-500)] bg-[var(--hide-900)] text-[var(--bone-300)]'
                        : 'border-[var(--hide-500)] bg-[var(--hide-900)] text-[var(--bone-300)]'
                }`}
              >
                {stage.label}
              </Link>
              {index < pipelineStages.length - 1 && <span className="font-mono text-[var(--brass-300)]">↓</span>}
            </div>
          );
        })}
      </div>

      <div className="mt-3 grid gap-2 text-[12px] font-mono text-[var(--bone-300)] md:grid-cols-3">
