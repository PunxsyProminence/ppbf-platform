...
Rejected as honest (verified, not reported): the Coach Workspace **Assessments** tab (`CoachWorkspace.tsx:2274-2280`, "Coming soon: Leadership assessment, communication effectiveness survey, teaching impact evaluation") has no matching table anywhere in `infra/azure/*.sql` (`assessment_protocols` is athlete physical-test protocol data, not coach evaluation) and no route -- correctly labeled planned. `sessionStatus = 'Live session tracking is not built yet.'` (`CoachWorkspace.tsx:542`) and the coach task list section is a deliberately-emptied honest state per its own inline comment (fabricated data was removed, not replaced with a feed) -- correct.

**Superseded:** the **coach goals** section was listed here as correctly-empty on the same grounds. It is no longer empty -- it now reads a real feed from `pilot.coach_development_goals` / `_activities` via `/api/pilot/coach/development`, and the inline comments this entry cited were deleted when that landed. Recorded rather than removed, because an audit reading this ledger to decide what still needs a backend would otherwise get the wrong answer for a surface that now has one -- which is the failure this ledger exists to prevent.
