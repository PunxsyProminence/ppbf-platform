...
- 🟡 **Correction (post-publication):** this bullet originally read "Reconciling `SHADOW_ML_ARCHITECTURE_SPEC.md` (still calls Scout Reports 'not implemented') against the actual shipped mechanism ... code has moved ahead of the doc here" — that overstated it. Re-checked directly: `SHADOW_ML_ARCHITECTURE_SPEC.md` §3.5/§5.5 is the more accurate of the two. The dedicated producer it specifies (`generateScoutReport`, `requestMode: 'profile'`, 72h TTL) was implemented and then **deliberately deleted** (see the tombstone comment in `apps/web/src/server/pilot/shadowHeavyBag.ts` above `sessionTypeToJobType`) because it never gained a caller; there is no `GET /api/pilot/shadow/scout-reports` route. What does exist is narrower than "a working mechanism": a `scout_report`-typed job can be enqueued through the chat endpoint's `sessionType` override and processed by the same `executeHeavyBagAsync` path as any other Heavy Bag session, but `/shadow/scout` (`apps/web/app/shadow/scout/page.tsx`) renders that job through the same **generic job list** UI as every other session type — there is no scout-specific results view, structured per-athlete report, or dedicated retrieval route. This is an open product question, not a decision this document should make: build the real per-athlete results view the page's title implies, or retitle/reshape the page to match what it actually does today (a generic job list that happens to include scout-typed jobs).
