...
**Status:** ACTIVE — Phase 1 architecture reconciliation  
**Owner:** Jason Neale  
...
For the PPBF research workflow, the current governed upstream archive baseline is:
...
The observed working structure is:
...
This supersedes stale product-documentation pointers that name `SHADOW AIML / 02 - Source Materials / Penn State Library Intake / ...` as the current archive destination.

It does **not** authorize deleting, moving, merging, or retiring any older Microsoft or Google tree. Older trees remain historical/provenance evidence until they are inventoried and separately dispositioned.
...
|---|---|---|
| Microsoft OneDrive / SharePoint | Durable original source, acquisition provenance, controlled research archive, duplicate/lineage preservation | Does not make a source citable merely because a file exists there |
...
| GitHub | Runtime implementation, contracts, derived evidence packages, import tooling, tests, and reproducible metadata | Must not contain licensed publisher PDFs or become the original archive |
| Google Drive | Design-lab work, coaching/skill masters where explicitly designated, handoffs, and candidate source material | Is not a direct SHADOW evidence authority and does not silently become nonprofit source |

Current executable GitHub code describes implementation behavior. Current Microsoft archive state describes original-file custody. Human evidence review determines citability. These authority classes must not be collapsed.

## 4. Required research flow

```text
source acquired
    -> preserve original and acquisition provenance
    -> R00 intake when classification or lineage is unresolved
    -> human subject classification to R01-R19, or R98 duplicate hold
    -> register SHADOW Library source as pending review / unverified
    -> optionally link to a research requirement
    -> create/process document and chunks
    -> generate current-model embeddings
    -> applicability and duplicate review
    -> index document
    -> human evidence verification and approval in /evidence
    -> SHADOW retrieval from organization shelf + __platform__ shelf
    -> human resolution of the research requirement when evidence actually answers it
```
...
The application classification taxonomy must align to the nineteen archive subject domains:
...
Classification is a human-correctable filing label. It is not an authority tier, an evidence grade, a promotion decision, or an instruction to move a file automatically.
...
Stable archive identifiers belong in protected configuration or stored metadata. Do not make public repository paths the only locator for a governed original.
...
The repository also contains `apps/web/seed-data/shadow-research/2026-08-08/`, including Penn State and multidiscipline integration artifacts. That package remains proposed/reference material unless a separate approved import and runtime verification establishes a later state.
...
- Licensed publisher PDFs remain in the governed internal archive and are never committed to public GitHub.

## 10. Phase 1 changes and remaining work

Phase 1 may:

- correct documentation and stale archive pointers;
- align the application taxonomy to R01-R19;
- add deterministic tests for taxonomy completeness and uniqueness;
...
- import the August 8 proposed package;
