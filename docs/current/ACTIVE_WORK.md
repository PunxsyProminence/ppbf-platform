...
2. **Quality assessment is a conformance check, not a score.** No 0–100 output. The model checks whether an identified action matches this gym's own coach-defined form for that named variant. The definition is owned by the gym's coaches and evolves as they refine it — SHADOW is not inventing what "good" means.
...
5. Separately surfaced during review, not part of this decision but relevant to it: `D10` in `SHADOW_PATTERN_FORMATION_CONTRACT.md` documents that SHADOW's medical-sensitivity safety gate is armed by a client-supplied boolean with no server-side inference, and is flagged there as the highest-severity item in that audit. Any video-derived recommendation touching return-to-play, clearance, or contact readiness should not be assumed to pass through that gate correctly until D10 is resolved.
