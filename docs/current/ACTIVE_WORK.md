...
| SHADOW near-miss text ALREADY DELIVERED to athlete/parent chats | The prospective leak is closed: owner decision 2026-09-26 restricted near-miss prompt context to coach and organization-admin authority, and `retrieveShadowContext` now gates on `DECISION_LOOP_ROLES` -- the same set `GET /near-misses` requires -- before the read. What is NOT closed is the text delivered before that gate existed: it is still visible in those conversations, and the conversation-history loader re-feeds recent turns into later prompts, so the model can repeat it after the fix. The affected population has not been measured and no environment was read. | An owner decision on whether delivered near-miss content is removed, from where, and under what retention rule. |
