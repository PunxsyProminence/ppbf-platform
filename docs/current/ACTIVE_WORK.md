...
| `BACKLOG-design-visuals-lane` | Owner decision 2026-08-17: the whole Design/visuals lane (`docs/VISUAL_BUILD_MAP.md` layers L1–L5) is parked while the owner squares away the three outstanding inputs — real gym photos + one committed staff photo (`apps/web/src/shared/gymPhotos.ts`), real coach sayings (`apps/web/components/gymSayings.ts`, ships empty on purpose — no invented lines), and a Canva social-card pick. This is a blanket park, not just the 🔒 rows — some layers (L1–L3, L5 template work, L4a/L4b) are technically 🔓 unblocked, but the owner asked to hold the whole lane rather than have sessions work the unblocked layers piecemeal while assets are being gathered. | Owner delivers the three inputs and un-parks the lane, or explicitly asks to resume just the unblocked layers before that. |
...
## Deploy status (as of 2026-08-19 docs check)

Confirmed directly against the GitHub Actions API during a docs-only stale-claims fix pass (no application code touched): the most recent **successful** `deploy-staging.yml` run and the most recent **successful** `deploy-production.yml` run both point at the same commit, the one that merged **PR #391** (`1da832e9`, 2026-08-16) — matching `docs/current/PRODUCTION_STATE.json`'s recorded `deployed_sha` for both environments. No newer successful run of either workflow exists as of this check. Everything merged to `main` after that point — roughly **PR #412 onward** (~120 commits, through at least #474 as of this note) — has **not** been confirmed deployed to either staging or production. This is a large batch: it includes, among other things, the sparring-attempt-contexts migration and the #459/#460/#461/#462 fixes this same pass corrected in `docs/capabilities/GATES.md`. Deploying is an owner action (workflow_dispatch with `confirm_sha`/`migrations_complete`/`release_digest`) and is out of scope for any agent to trigger.
