...
| `BACKLOG-session-objective-link` | The build order's PR F asks a coach to identify three things about a session: which development block it supported, which OBJECTIVES it addressed, and which actual activities occurred. The first and third shipped (`pilot.session_run_development_block_links`, plus the run's own `deviation_note` / `what_worked` / `what_did_not` read back verbatim). The second needs `pilot.athlete_development_block_objectives`, which is PR #762's table in another lane and is NOT on main — building against an unmerged migration would make this one undeployable. Nothing forecloses it: an objective link is a second link table, or a nullable column on the existing one. | #762 merges. The link table is then additive, and the reads already carry the block id the objectives hang off. |
