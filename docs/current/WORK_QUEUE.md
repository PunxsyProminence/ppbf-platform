...
| T-002 | P1 | Covering coach cannot access an athlete they don't own | session B (remote), claimed 2026-08-06 | build | CLAIMED | none | `access.ts`, new migration | medium — auth + schema | will land on PR [#238](https://github.com/PunxsyProminence/ppbf-platform/pull/238)'s branch (session B is single-branch constrained; deviation from the ticket's Lane A branch naming, declared here and in the PR) | — | — | Coverage model decision + rejected alternative will be stated in the PR per the ticket | 2026-08-06 |
