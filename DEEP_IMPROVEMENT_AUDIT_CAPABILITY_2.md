...
|----|-------|----------|-----------------|
| **P0-1** | ✅ FIXED: Create route restricted to org-admin only | Previously allowed coaches; now restricted to `['organization_admin']` | Decision: Roster creation is an admin responsibility. Coaches manage existing athletes but don't add them to the system. Tests updated: coach now correctly rejected with 403. UI note added to People tab. |
